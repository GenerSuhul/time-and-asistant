...
async function processRegistrations() {
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .select("id,device_id,encrypted_key,iv,attempts,devices:device_id(name,device_identifier)")
    ...
    for (const request of data ?? []) {
      ...
      try {
        ...
        await supabase.from("device_registration_requests").delete().eq("id", request.id);
