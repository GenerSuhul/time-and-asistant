...
import {
  ...
  TablePagination, TableRow, Tabs, TextField, Tooltip, Typography, useMediaQuery
} from "@mui/material";
...
import {
  deviceCommandErrorMessage, deviceCommandLabel, technicalReference
} from "../lib/deviceCommandMessages";
...
const baseForm: EmployeeForm = { id: "", company_id: "", branch_id: "", department_id: "", employee_code: "", external_employee_id: "", hikvision_employee_no: "",
  full_name: "", email: "", phone: "", document_number: "", status: "active", card_number: "", pin_enabled: false, hikvision_is_admin: false,
...
function formFromEmployee(row: Row, assignments: Row[]): EmployeeForm {
  return { ...baseForm, id: row.id, company_id: row.company_id ?? "", branch_id: row.branch_id ?? "", department_id: row.department_id ?? "",
    ...
    card_number: row.card_number ?? "", pin_enabled: Boolean(row.pin_enabled), hikvision_is_admin: Boolean(row.hikvision_is_admin), hired_at: row.hired_at ?? today,
...
export function EmployeeManagementPage() {
  ...
  const commandFailures = employees.data?.commands ?? [];
  ...
  function employeePayload() {
    return { company_id: form.company_id, branch_id: form.branch_id || null, department_id: form.department_id || null,
      ...
      pin_enabled: form.pin_enabled, hikvision_is_admin: form.hikvision_is_admin,
      hired_at: form.hired_at || null, terminated_at: form.terminated_at || null,
  ...
  const repairEmployee = useMutation({
    mutationFn: (employeeId: string) => invokeEdge("admin-employees", { action: "repair_employee_credentials", employee_id: employeeId }),
    onSuccess: () => setNotice("Verificación y reparación encoladas por dispositivo.")
  });
  const retryFailed = useMutation({
    mutationFn: (commandIds?: string[]) => invokeEdge("admin-employees", { action: "retry_failed_commands", command_ids: commandIds }),
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ["employees-management-v2"] });
      setNotice("Se reintentaron únicamente los errores activos seleccionados.");
    }
  });
  const resolveCommand = useMutation({
    mutationFn: (commandId: string) => invokeEdge("admin-employees", { action: "resolve_command", command_id: commandId }),
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ["employees-management-v2"] });
      setNotice("El error quedó resuelto después de verificar el estado real.");
    }
  });
  ...
  return <Stack spacing={2}>
    ...
    <Grid2 container spacing={1.5}>
      ...
      <Grid2 size={{ xs: 12, md: 9.6 }}><Stack spacing={1.5}>
        ...
        {(employees.error || catalog.error || saveEmployee.error || deleteEmployee.error || syncAll.error || repairEmployee.error || retryFailed.error || resolveCommand.error) &&
          <Alert severity="error">{String(employees.error?.message ?? catalog.error?.message ?? saveEmployee.error?.message ?? deleteEmployee.error?.message ?? syncAll.error?.message ?? repairEmployee.error?.message ?? retryFailed.error?.message ?? resolveCommand.error?.message)}</Alert>}
        {notice && <Alert severity={notice.includes("requiere revisión") ? "warning" : "success"} onClose={() => setNotice("")}>{notice}</Alert>}
        {commandFailures.length > 0 && <Alert severity={commandFailures.some((item) => item.error_code === "HIKVISION_DEVICE_HARDWARE_ERROR") ? "error" : "warning"}
          action={<Stack direction="row" gap={0.5}><Button color="inherit" size="small" onClick={() => setErrorsOpen(true)}>Ver detalles</Button><Button color="inherit" size="small" disabled={retryFailed.isPending} onClick={() => retryFailed.mutate(commandFailures.map((item) => item.id))}>Reintentar fallidos</Button></Stack>}>
          <Typography fontWeight={700}>{commandFailures.length} {commandFailures.length === 1 ? "alerta activa" : "alertas activas"} por dispositivo</Typography>
          <Typography variant="body2">El histórico resuelto no se muestra como incidente actual.</Typography>
        </Alert>}
        <TableContainer component={Paper} variant="outlined"><Table size="small">
          <TableHead><TableRow><TableCell>ID / persona</TableCell><TableCell>Resumen</TableCell><TableCell>Estado real por dispositivo</TableCell><TableCell>Estado</TableCell><TableCell align="right">Acciones</TableCell></TableRow></TableHead>
          <TableBody>{(employees.data?.people ?? []).map((row) => <TableRow key={row.id} hover>
            <TableCell><Stack direction="row" gap={1} alignItems="center"><Tooltip title="Foto no registrada"><Avatar sx={{ width: 28, height: 28 }}><BadgeIcon sx={{ fontSize: 17 }} /></Avatar></Tooltip><Box><Stack direction="row" gap={0.5} alignItems="center"><Typography fontWeight={700}>{row.full_name}</Typography>{row.hikvision_is_admin && <Tooltip title="Administrador local en dispositivos"><AdminPanelSettingsIcon color="primary" sx={{ fontSize: 17 }} /></Tooltip>}</Stack><Typography variant="caption" display="block">Código: {row.employee_code} · Hikvision: {row.hikvision_employee_no}</Typography><Typography variant="caption">{relationLabel(row.branches) || "Sin sucursal"}</Typography></Box></Stack></TableCell>
            <TableCell><Stack direction="row" gap={1}><CompactCredentialCount kind="card" count={row.card_number ? 1 : 0} /><CompactCredentialCount kind="fingerprint" count={row.fingerprint_count ?? 0} /><CompactCredentialCount kind="face" count={row.face_status === "enrolled" ? 1 : 0} /></Stack></TableCell>
            <TableCell><EmployeeDeviceSummary assignments={assignmentsByEmployee[row.id] ?? []} credentialsByDevice={credentialsByEmployeeDevice} auditsByDevice={latestAuditByEmployeeDevice} employeeId={row.id} expectedEmployeeNo={row.hikvision_employee_no} /></TableCell>
            <TableCell><StatusChip value={row.status} /></TableCell>
            <TableCell align="right"><Tooltip title="Verificar y reparar credenciales"><span><IconButton color="primary" disabled={repairEmployee.isPending} onClick={() => repairEmployee.mutate(row.id)}><BuildCircleOutlinedIcon fontSize="small" /></IconButton></span></Tooltip><IconButton onClick={() => startEdit(row)}><EditIcon fontSize="small" /></IconButton><IconButton color="error" onClick={() => setDeleteTarget(row)}><DeleteIcon fontSize="small" /></IconButton></TableCell>
          </TableRow>)}</TableBody>
          <TablePagination component="div" count={employees.data?.total ?? 0} page={page} onPageChange={(_, value) => setPage(value)} rowsPerPage={pageSize} onRowsPerPageChange={(event) => setPageSize(Number(event.target.value))} rowsPerPageOptions={[20, 50, 100]} labelRowsPerPage="Filas" labelDisplayedRows={({ from, to, count }) => `${from}-${to} de ${count}`} />
    ...
    <Dialog open={errorsOpen} onClose={() => setErrorsOpen(false)} fullWidth maxWidth="md"><DialogTitle>Alertas activas de sincronización</DialogTitle><DialogContent dividers><Stack spacing={1}>
      {commandFailures.map((command) => <Paper key={command.id} variant="outlined" sx={{ p: 1.25 }}><Stack direction={{ xs: "column", sm: "row" }} justifyContent="space-between" gap={1}>
        <Box><Stack direction="row" gap={0.75} alignItems="center"><ErrorOutlineIcon color="error" fontSize="small" /><Typography fontWeight={700}>{relationLabel(command.devices) || command.device_id}</Typography><Chip size="small" label={deviceCommandLabel(command.command_type)} /></Stack><Typography variant="body2" sx={{ mt: 0.5 }}>{deviceCommandErrorMessage(command)}</Typography><Tooltip title={`Job ${command.id} · Trace ${command.payload?.trace_id || command.id}`}><Typography variant="caption" color="text.secondary">Referencia técnica: {technicalReference(command)}</Typography></Tooltip></Box>
        <Stack direction="row" gap={0.5} alignItems="center"><Button size="small" disabled={retryFailed.isPending} onClick={() => retryFailed.mutate([command.id])}>Reintentar</Button><Button size="small" disabled={resolveCommand.isPending} onClick={() => resolveCommand.mutate(command.id)}>Marcar resuelto</Button></Stack>
      </Stack></Paper>)}
    </Stack></DialogContent><DialogActions><Button onClick={() => setErrorsOpen(false)}>Cerrar</Button></DialogActions></Dialog>
  </Stack>;
}
...
function EmployeeDeviceSummary({ assignments, credentialsByDevice, auditsByDevice, employeeId, expectedEmployeeNo }: {
  assignments: Row[]; credentialsByDevice: Record<string, Row[]>; auditsByDevice: Record<string, Row>;
  employeeId: string; expectedEmployeeNo: string;
}) {
  const [detailOpen, setDetailOpen] = useState(false);
  const renderAssignment = (assignment: Row) => <DeviceCredentialSummary key={assignment.id} assignment={assignment}
    credentials={credentialsByDevice[`${employeeId}:${assignment.device_id}`] ?? []}
    audit={auditsByDevice[`${employeeId}:${assignment.device_id}`]} expectedEmployeeNo={expectedEmployeeNo} />;
  return <Stack gap={0.5}>
    {assignments.slice(0, 3).map(renderAssignment)}
    {assignments.length > 3 && <Button size="small" startIcon={<MoreHorizIcon />} sx={{ alignSelf: "flex-start" }} onClick={() => setDetailOpen(true)}>Ver detalle ({assignments.length})</Button>}
    <Dialog open={detailOpen} onClose={() => setDetailOpen(false)} fullWidth maxWidth="sm"><DialogTitle>Estado real por dispositivo</DialogTitle><DialogContent dividers><Stack gap={0.75}>{assignments.map(renderAssignment)}</Stack></DialogContent><DialogActions><Button onClick={() => setDetailOpen(false)}>Cerrar</Button></DialogActions></Dialog>
  </Stack>;
}
...
function DeviceCredentialSummary({ assignment, credentials, audit, expectedEmployeeNo }: { assignment: Row; credentials: Row[]; audit?: Row; expectedEmployeeNo: string }) {
  ...
  return <Paper variant="outlined" sx={{ px: 0.75, py: 0.35, minWidth: 270, maxWidth: 350 }}>
    <Stack direction="row" gap={0.35} alignItems="center">
      <Tooltip title={mismatch ? `employeeNo físico ${assignment.external_person_id}; esperado ${expectedEmployeeNo}` : assignment.devices?.status === "offline" ? "Dispositivo fuera de línea" : "Dispositivo verificado"}>
        <Typography variant="caption" fontWeight={700} noWrap color={mismatch ? "error" : "text.primary"} sx={{ width: 150 }}>{assignment.devices?.name ?? assignment.device_id}</Typography>
      </Tooltip>
      <CredentialStateIcon kind="person" state={byType.person} fallback={assignment.sync_status} audit={audit} />
      <CredentialStateIcon kind="role" state={byType.role} audit={audit} />
      <CredentialStateIcon kind="card" state={byType.card} audit={audit} />
      <CredentialStateIcon kind="fingerprint" state={byType.fingerprint} audit={audit} />
    </Stack>
  </Paper>;
}
...
function CredentialDevicePanel({ credentials, audit }: { credentials: Row[]; audit?: Row | null }) {
  const byType = Object.fromEntries(credentials.map((item) => [item.credential_type, item]));
  const fingerprintLimitation = byType.fingerprint?.metadata?.limitation;
  const fingerprintError = byType.fingerprint?.last_error || audit?.sanitized_error;
  return <Paper variant="outlined" sx={{ p: 1.25 }}><Stack spacing={0.75}>
    ...
    <Stack direction="row" gap={1}><CredentialStateIcon kind="person" state={byType.person} audit={audit ?? undefined} /><CredentialStateIcon kind="role" state={byType.role} audit={audit ?? undefined} /><CredentialStateIcon kind="card" state={byType.card} audit={audit ?? undefined} /><CredentialStateIcon kind="fingerprint" state={byType.fingerprint} audit={audit ?? undefined} /></Stack>
    {fingerprintLimitation && <Alert severity="info"><Typography variant="body2">{fingerprintLimitation} La huella confirmada sí puede utilizarse para marcar.</Typography></Alert>}
    {fingerprintError && !fingerprintLimitation && <Alert severity="error"><Typography variant="body2">{deviceCommandErrorMessage(fingerprintError)}</Typography><Typography variant="caption">Referencia técnica: {String(byType.fingerprint?.trace_id || audit?.trace_id || "SIN-REF").slice(0, 8).toUpperCase()}</Typography></Alert>}
  </Stack></Paper>;
}

function CredentialStateIcon({ kind, state, fallback = "pending", audit }: { kind: "person" | "role" | "card" | "fingerprint"; state?: Row; fallback?: string; audit?: Row }) {
  const status = state?.status ?? fallback;
  const deviceLimited = Boolean(state?.metadata?.device_limited);
  const color = deviceLimited ? "warning.main" : ["captured", "synced"].includes(status) ? "success.main" : status === "failed" ? "error.main" : ["pending", "processing"].includes(status) ? "warning.main" : "text.disabled";
  const verifiedCount = Number(state?.verified_count ?? 0);
  const label = kind === "person" ? "Persona" : kind === "role" ? "Rol" : kind === "card" ? "Tarjeta" : "Huella";
  const Icon = kind === "person" ? PersonIcon : kind === "role" ? AdminPanelSettingsIcon : kind === "card" ? CreditCardIcon : FingerprintIcon;
  const verifiedAt = state?.last_verified_at ? new Date(state.last_verified_at).toLocaleString() : "sin verificar";
  const error = state?.last_error || audit?.sanitized_error;
  const traceId = state?.trace_id || audit?.trace_id;
  return <Tooltip arrow title={<Box><Typography variant="caption" fontWeight={700}>{label}: {deviceLimited ? "sincronizada con límite del equipo" : credentialStatusLabel(status)}{verifiedCount > 0 ? ` (${verifiedCount})` : ""}</Typography><Typography variant="caption" display="block">Última verificación: {verifiedAt}</Typography>{deviceLimited && <Typography variant="caption" display="block">Este equipo conserva una huella por persona; la huella confirmada está operativa.</Typography>}{error && !deviceLimited && <Typography variant="caption" display="block" color="error.light">Detalle: {deviceCommandErrorMessage(error)}</Typography>}{traceId && <Typography variant="caption" display="block">Referencia: {String(traceId).slice(0, 8).toUpperCase()}</Typography>}</Box>}>
    <Box component="span" sx={{ display: "inline-flex", color, p: 0.35 }}><Icon sx={{ fontSize: 19 }} /></Box>
  </Tooltip>;
}

function CompactCredentialCount({ kind, count }: { kind: "card" | "fingerprint" | "face"; count: number }) {
  const Icon = kind === "card" ? CreditCardIcon : kind === "fingerprint" ? FingerprintIcon : FaceRetouchingNaturalIcon;
  const label = kind === "card" ? "Tarjetas" : kind === "fingerprint" ? "Huellas verificadas" : "Rostros";
  return <Tooltip title={`${label}: ${count}`}><Stack direction="row" gap={0.25} alignItems="center" color={count > 0 ? "success.main" : "text.disabled"}><Icon sx={{ fontSize: 18 }} /><Typography variant="caption" fontWeight={700}>{count}</Typography></Stack></Tooltip>;
}
