    ...
    <Grid2 container spacing={1.5}>
      ...
      <Grid2 size={{ xs: 12, md: 9.6 }}><Stack spacing={1.5}>
        ...
        <TableContainer component={Paper} variant="outlined"><Table size="small">
          <TableHead><TableRow><TableCell>Foto</TableCell><TableCell>ID / persona</TableCell><TableCell>Resumen</TableCell><TableCell>Estado real por dispositivo</TableCell><TableCell>Estado</TableCell><TableCell align="right">Acciones</TableCell></TableRow></TableHead>
          <TableBody>{(employees.data?.people ?? []).map((row) => <TableRow key={row.id} hover>
            <TableCell><Avatar><BadgeIcon /></Avatar></TableCell>
            <TableCell><Typography fontWeight={700}>{row.full_name}</Typography><Typography variant="caption" display="block">Código: {row.employee_code} · Hikvision: {row.hikvision_employee_no}</Typography><Typography variant="caption">{relationLabel(row.branches) || "Sin sucursal"}</Typography></TableCell>
...
function DeviceCredentialSummary({ assignment, credentials, audit, expectedEmployeeNo }: { assignment: Row; credentials: Row[]; audit?: Row; expectedEmployeeNo: string }) {
  ...
  return <Paper variant="outlined" sx={{ px: 0.75, py: 0.35, minWidth: 245, maxWidth: 330 }}>
...
function CredentialDevicePanel({ credentials, audit }: { credentials: Row[]; audit?: Row | null }) {
  ...
  return <Paper variant="outlined" sx={{ p: 1.25 }}><Stack spacing={0.75}>
    ...
    <Stack direction="row" gap={1}><CredentialStateIcon kind="person" state={byType.person} audit={audit ?? undefined} /><CredentialStateIcon kind="card" state={byType.card} audit={audit ?? undefined} /><CredentialStateIcon kind="fingerprint" state={byType.fingerprint} audit={audit ?? undefined} /></Stack>
...
function CredentialStateIcon({ kind, state, fallback = "pending", audit }: { kind: "person" | "card" | "fingerprint"; state?: Row; fallback?: string; audit?: Row }) {
  ...
  const label = kind === "person" ? "Persona" : kind === "card" ? "Tarjeta" : "Huella";
  const Icon = kind === "person" ? PersonIcon : kind === "card" ? CreditCardIcon : FingerprintIcon;
