...
type EmployeeForm = {
  ...
  external_employee_id: string; full_name: string; email: string; phone: string; document_number: string;
...
type CreationSession = { id: string; status: string; trace_id?: string; error_code?: string; error_message?: string };

const today = new Date().toISOString().slice(0, 10);
const defaultValidTo = "2036-12-31";
const baseForm: EmployeeForm = { id: "", company_id: "", branch_id: "", department_id: "", employee_code: "", external_employee_id: "",
...
function formFromEmployee(row: Row, assignments: Row[]): EmployeeForm {
  return { ...baseForm, id: row.id, company_id: row.company_id ?? "", branch_id: row.branch_id ?? "", department_id: row.department_id ?? "",
    employee_code: row.employee_code ?? "", external_employee_id: row.external_employee_id ?? "", full_name: row.full_name ?? "",
...
export function EmployeeManagementPage() {
  ...
  const employees = useQuery({
    ...
    queryFn: async () => {
      ...
      if (safeSearch) request = request.or(`employee_code.ilike.%${safeSearch}%,external_employee_id.ilike.%${safeSearch}%,full_name.ilike.%${safeSearch}%`);
      const peopleResult = await request;
      if (peopleResult.error) throw peopleResult.error;
      const ids = (peopleResult.data ?? []).map((item) => item.id);
      const [assignments, commands] = await Promise.all([
      ...
      return { people: peopleResult.data ?? [], total: peopleResult.count ?? 0, assignments: assignments.data ?? [], commands: commands.data ?? [] };
  ...
  useEffect(() => {
    ...
    const apply = (value: Row) => setEnrollmentState(value);
  ...
  function employeePayload() {
    return { company_id: form.company_id, branch_id: form.branch_id || null, department_id: form.department_id || null,
      employee_code: form.employee_code.trim(), external_employee_id: form.external_employee_id.trim() || null,
  ...
  const enrollFingerprint = useMutation({ mutationFn: async () => {
    ...
    if (form.id) return invokeEdge("admin-employees", { action: "enroll_fingerprint", employee_id: form.id, device_id: fingerDeviceId, finger_no: fingerNo });
  ...
  return <Stack spacing={2}>
    ...
    <Grid2 container spacing={1.5}>
      <Grid2 size={{ xs: 12, md: 3 }}><Paper variant="outlined" sx={{ p: 1.5, maxHeight: 620, overflowY: "auto" }}><Typography fontWeight={700} sx={{ mb: 1 }}>Empresa / sucursal / departamento</Typography><Button fullWidth size="small" variant={!companyFilter && !branchFilter ? "contained" : "text"} onClick={() => { setCompanyFilter(""); setBranchFilter(""); setDepartmentFilter(""); }}>Todas las empresas</Button>{(catalog.data?.companies ?? []).map((company) => <Box key={company.id} sx={{ mt: 1 }}><Button fullWidth sx={{ justifyContent: "flex-start" }} variant={companyFilter === company.id && !branchFilter ? "contained" : "text"} onClick={() => { setCompanyFilter(company.id); setBranchFilter(""); setDepartmentFilter(""); }}>{company.name}</Button>{(catalog.data?.branches ?? []).filter((branch) => branch.company_id === company.id).map((branch) => <Box key={branch.id} sx={{ pl: 1 }}><Button fullWidth size="small" sx={{ justifyContent: "flex-start" }} variant={branchFilter === branch.id && !departmentFilter ? "outlined" : "text"} onClick={() => { setCompanyFilter(company.id); setBranchFilter(branch.id); setDepartmentFilter(""); }}>{branch.name}</Button>{branchFilter === branch.id && visibleDepartments.filter((dept) => dept.company_id === company.id).map((dept) => <Button key={dept.id} fullWidth size="small" sx={{ justifyContent: "flex-start", pl: 3 }} variant={departmentFilter === dept.id ? "contained" : "text"} onClick={() => setDepartmentFilter(dept.id)}>{dept.name}</Button>)}</Box>)}</Box>)}</Paper></Grid2>
      <Grid2 size={{ xs: 12, md: 9 }}><Stack spacing={1.5}>
        ...
        {commandFailures.length > 0 && <Alert severity="error"><Typography fontWeight={700}>Comandos con error/reintento:</Typography>{commandFailures.map((command) => <Typography key={command.id} variant="body2">{command.command_type} · {relationLabel(command.devices) || command.device_id} · {command.status}: {command.error_message}</Typography>)}</Alert>}
        <TableContainer component={Paper} variant="outlined"><Table size="small"><TableHead><TableRow><TableCell>Foto</TableCell><TableCell>ID / persona</TableCell><TableCell>Departamento</TableCell><TableCell>Credenciales</TableCell><TableCell>Dispositivos / sync</TableCell><TableCell>Estado</TableCell><TableCell align="right">Acciones</TableCell></TableRow></TableHead><TableBody>{(employees.data?.people ?? []).map((row) => <TableRow key={row.id} hover><TableCell><Avatar><BadgeIcon /></Avatar></TableCell><TableCell><Typography fontWeight={700}>{row.full_name}</Typography><Typography variant="caption">{row.external_employee_id || row.employee_code} · {relationLabel(row.branches) || "Sin sucursal"}</Typography></TableCell><TableCell>{relationLabel(row.departments) || "-"}</TableCell><TableCell><Stack direction="row" gap={0.5}><Chip size="small" icon={<CreditCardIcon />} label={row.card_number ? "1" : "0"} /><Chip size="small" icon={<FingerprintIcon />} label={row.fingerprint_count ?? 0} /><Chip size="small" label={row.face_status === "enrolled" ? "Rostro 1" : "Rostro 0"} /></Stack></TableCell><TableCell><Stack direction="row" gap={0.5} flexWrap="wrap">{(assignmentsByEmployee[row.id] ?? []).map((item) => <Chip key={item.id} size="small" color={item.sync_status === "failed" ? "error" : item.sync_status === "success" ? "success" : "default"} label={`${item.devices?.name ?? item.device_id} · ${item.sync_status}`} title={item.last_error ?? ""} />)}</Stack></TableCell><TableCell><StatusChip value={row.status} /></TableCell><TableCell align="right"><IconButton onClick={() => startEdit(row)}><EditIcon fontSize="small" /></IconButton><IconButton color="error" onClick={() => setDeleteTarget(row)}><DeleteIcon fontSize="small" /></IconButton></TableCell></TableRow>)}</TableBody></Table><TablePagination component="div" count={employees.data?.total ?? 0} page={page} onPageChange={(_, value) => setPage(value)} rowsPerPage={pageSize} onRowsPerPageChange={(event) => setPageSize(Number(event.target.value))} rowsPerPageOptions={[20, 50, 100]} labelRowsPerPage="Filas" labelDisplayedRows={({ from, to, count }) => `${from}-${to} de ${count}`} /></TableContainer>
      </Stack></Grid2>
    </Grid2>
    ...
    <Drawer anchor="right" open={open && credentialOpen} onClose={() => setCredentialOpen(false)} sx={{ zIndex: (value) => value.zIndex.modal + 1 }} PaperProps={{ sx: { width: { xs: "100%", sm: 440 }, p: 2, overflowY: "auto" } }}><Stack spacing={2}><Stack direction="row" justifyContent="space-between"><Typography variant="h6">Administración de credencial</Typography><IconButton onClick={() => setCredentialOpen(false)}><CloseIcon /></IconButton></Stack><Divider /><Typography fontWeight={700}>Tarjeta y PIN</Typography><TextField label="Número de tarjeta" value={form.card_number} onChange={(event) => setForm((current) => ({ ...current, card_number: event.target.value }))} /><FormControlLabel control={<Checkbox checked={form.pin_enabled} onChange={(event) => setForm((current) => ({ ...current, pin_enabled: event.target.checked }))} />} label="PIN habilitado" /><Divider /><Typography fontWeight={700}>Huella dactilar</Typography>{!form.target_device_ids.length && !form.id && <Alert severity="warning">Selecciona un dispositivo destino antes de capturar.</Alert>}<TextField select label="Dispositivo" value={fingerDeviceId} onChange={(event) => setFingerDeviceId(event.target.value)} disabled={!fingerprintDevices.length} SelectProps={{ MenuProps: { sx: { zIndex: (value) => value.zIndex.modal + 2 } } }}><MenuItem value="">Selecciona dispositivo</MenuItem>{fingerprintDevices.map((device) => <MenuItem key={device.id} value={device.id}>{device.name} ({device.status})</MenuItem>)}</TextField><TextField select label="Dedo" value={fingerNo} onChange={(event) => setFingerNo(Number(event.target.value))} SelectProps={{ MenuProps: { sx: { zIndex: (value) => value.zIndex.modal + 2 } } }}>{Array.from({ length: 10 }, (_, index) => index + 1).map((value) => <MenuItem key={value} value={value}>Huella {value}</MenuItem>)}</TextField><Button variant="contained" startIcon={<FingerprintIcon />} disabled={enrollFingerprint.isPending || !fingerDeviceId || creationSession?.status === "enrolling"} onClick={() => enrollFingerprint.mutate()}>Capturar huella</Button>{enrollFingerprint.error && <Alert severity="error">{enrollFingerprint.error.message}</Alert>}{(creationSession || enrollmentState) && <Alert severity={creationSession?.status === "failed" ? "error" : creationSession?.status === "captured" ? "success" : "info"}><Typography fontWeight={700}>{enrollmentState?.status_detail ?? statusLabel(creationSession?.status)}</Typography>{enrollmentState?.error_message && <Typography variant="body2">{enrollmentState.error_message}</Typography>}<Typography variant="caption">Trace: {creationSession?.trace_id || enrollmentState?.trace_id || "pendiente"}</Typography></Alert>}<Alert severity="info">El template existe solo en memoria durante CaptureFingerPrint → FingerPrintDownload; nunca llega al navegador ni se persiste.</Alert></Stack></Drawer>
