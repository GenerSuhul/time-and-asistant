  ...
  return <Stack spacing={2}>
    ...
    <Dialog open={errorsOpen} onClose={() => setErrorsOpen(false)} fullWidth maxWidth="md"><DialogTitle>Alertas activas de sincronización</DialogTitle><DialogContent dividers><Stack spacing={1}>
      {commandFailures.map((command) => <Paper key={command.id} variant="outlined" sx={{ p: 1.25 }}><Stack direction={{ xs: "column", sm: "row" }} justifyContent="space-between" gap={1}>
        <Box><Stack direction="row" gap={0.75} alignItems="center"><ErrorOutlineIcon color="error" fontSize="small" /><Typography fontWeight={700}>{relationLabel(command.devices) || command.device_id}</Typography><Chip size="small" label={command.command_type} /></Stack><Typography variant="body2" sx={{ mt: 0.5 }}>{command.error_message}</Typography><Typography variant="caption" color="text.secondary">Job {command.id} · Trace {command.payload?.trace_id || command.id}</Typography></Box>
...
function CredentialStateIcon({ kind, state, fallback = "pending", audit }: { kind: "person" | "role" | "card" | "fingerprint"; state?: Row; fallback?: string; audit?: Row }) {
  ...
  const color = ["captured", "synced"].includes(status) ? "success.main" : status === "failed" ? "error.main" : ["pending", "processing"].includes(status) ? "warning.main" : "text.disabled";
  ...
  return <Tooltip arrow title={<Box><Typography variant="caption" fontWeight={700}>{label}: {credentialStatusLabel(status)}{verifiedCount > 0 ? ` (${verifiedCount})` : ""}</Typography><Typography variant="caption" display="block">Última verificación: {verifiedAt}</Typography>{error && <Typography variant="caption" display="block" color="error.light">Error: {error}</Typography>}{traceId && <Typography variant="caption" display="block">Trace: {traceId}</Typography>}</Box>}>
